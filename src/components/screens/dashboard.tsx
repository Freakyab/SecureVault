  ...
  const categories = useMemo(
    () =>
      CREDENTIAL_CATEGORIES.map((category) => ({
        ...category,
        count: activeCredentials.filter(
          (credential) => credential.category === category.id,
        ).length,
      })),
    [activeCredentials],
  );
      ...
      <ScrollView
        ...
        {searched ? null : (
          <Animated.View
            entering={FadeInDown.duration(
              theme.motion.duration.cardExpand,
            ).delay(80)}
            style={s.statsRow}>
            ...
            <View style={s.statCol}>
              <Text style={[p.statValue, { color: theme.colors.success }]}>
                {animatedScore}%
              </Text>
            ...
            <View style={s.statCol}>
              <Text
                style={[
                  ...
                  {
                    color:
                      alerts > 0 ? theme.colors.warning : theme.colors.text,
                  },
                ]}>
                {alerts}
              </Text>
            ...
            <View style={s.grid}>
              {categories.map((category, index) => (
                <Animated.View
                  ...
                  entering={FadeInDown.duration(
                    theme.motion.duration.cardExpand,
                  ).delay(120 + index * theme.motion.stagger.list)}
                  style={s.gridItem}>
                  <CategoryTile
        ...
        <Text
          style={[
            p.sectionTitle,
            {
              marginTop: theme.layout.sectionSpacingLarge,
          ...
          {visibleCredentials.length > 0 ? (
            visibleCredentials.map((credential, index) => (
              <Animated.View
                ...
                entering={FadeInDown.duration(
                  theme.motion.duration.cardExpand,
                ).delay(index * theme.motion.stagger.list)}>
                <CredentialRow
        ...
        <Animated.View
          entering={FadeInDown.duration(theme.motion.duration.navigation).delay(
            120,
          )}>
          <PressableScale
            ...
            <LinearGradient
              ...
              <LinearGradient
                ...
                style={p.pillButton}>
                <Text style={p.pillButtonText}>
                  {alerts > 0 ? "Resolve Now" : "View Report"}
                </Text>
                <ChevronRight
                  size={16}
